...
#  Rekomendasi: Association Rules (mlxtend) + LLM (Claude API)
# ============================================================
...
def _render_association_insights(r: dict) -> None:
    ...
    for alat, arah, conf, lift, is_match in relevant:
        ...
        st.markdown(
            ...
            f'<span style="font-weight:700;font-size:.95rem">{label_emoji} {alat_label}</span>'
            ...
            f'Homes labeled <b>{label}</b> tend to use <b>{alat_label}</b> '
            f'{"above" if arah == "high" else "below"} average '
...
# ── LLM Recommendations ───────────────────────────────────────
def _build_llm_prompt(r: dict) -> str:
    label      = r["label"]
    kwh        = r["kwh_bulan"]
    n          = r["n_penghuni"]
    kpo        = r["kwh_per_orang_bulan"]
    est_bulan  = r["est_bulan"]
    props      = r["props"]

    # Top 4 alat berdasarkan proporsi
    top_alat = sorted(
        [(ap["label"], props.get(ap["id"], 0), est_bulan.get(ap["id"], 0))
         for ap in APPLIANCES if props.get(ap["id"], 0) > 0],
        key=lambda x: x[1], reverse=True
    )[:4]

    top_str = "\n".join(
        f"  - {name}: {pct*100:.1f}% of usage (~{kwh_m:.1f} kWh/month)"
        for name, pct, kwh_m in top_alat
    )

    assoc = _ALL_RULES.get(label, [])
    assoc_str = "\n".join(
        f"  - Homes labeled '{label}' tend to use {arah_alat.replace('_', ' ', 1)} "
        f"(confidence {conf:.0%}, lift {lift:.2f})"
        for arah_alat, conf, lift in assoc[:3]
    )

    return f"""You are EcoWatt AI, an energy efficiency advisor.

A household has been classified as **{label.upper()}** energy consumption.

Household profile:
- Monthly usage: {kwh} kWh
- Residents: {n} people
- Usage per person: {kpo:.1f} kWh/month

Top appliances by consumption proportion:
{top_str}

Association rule patterns found from dataset mining for '{label}' homes:
{assoc_str}

Based on this profile and the association rule patterns above, generate exactly 3 personalized, actionable energy-saving recommendations.

Respond in JSON only, no preamble, no markdown fences. Format:
[
  {{"appliance": "<appliance name>", "tip": "<one specific actionable tip, max 2 sentences>", "estimated_saving": "<rough % or Rp estimate if possible>"}},
  {{"appliance": "<appliance name>", "tip": "<tip>", "estimated_saving": "<estimate>"}},
  {{"appliance": "<appliance name>", "tip": "<tip>", "estimated_saving": "<estimate>"}}
]"""


def _render_llm_recommendations(r: dict) -> None:
    """Generate rekomendasi personal via Claude API."""
    st.markdown(
        '<div style="font-size:1.1rem;font-weight:800;color:#1A2420;margin:16px 0 4px">'
        '🤖 AI-Generated Recommendations</div>'
        '<p style="font-size:.87rem;color:#6B8F78;font-weight:500;margin-bottom:14px">'
        'Personalized tips generated by AI based on your energy profile '
        'and patterns from the dataset.</p>',
        unsafe_allow_html=True,
    )

    # Cache per session agar tidak hit API berkali-kali
    cache_key = f"llm_reco_{r['label']}_{r['kwh_bulan']}_{r['n_penghuni']}"
    if cache_key in st.session_state:
        _display_llm_cards(st.session_state[cache_key])
        return

    with st.spinner("🤖 EcoWatt AI is generating personalized recommendations..."):
        try:
            import requests
            prompt = _build_llm_prompt(r)
            ...
            recos = json.loads(raw)
            st.session_state[cache_key] = recos
            _display_llm_cards(recos)
        except Exception as e:
            st.warning(f"⚠️ Could not generate AI recommendations: {e}")
            _render_fallback_recommendations(r)


def _display_llm_cards(recos: list) -> None:
    icons = ["💡", "🌿", "⚡"]
    for i, rec in enumerate(recos):
        icon     = icons[i % len(icons)]
        appliance = rec.get("appliance", "—")
        tip       = rec.get("tip", "")
        saving    = rec.get("estimated_saving", "")
        st.markdown(
            f'<div class="reco-card">'
            f'<div class="reco-icon">{icon}</div>'
            f'<div>'
            f'<div class="reco-title">{appliance}</div>'
            f'{("<div class=\"reco-pct\">💰 Estimated saving: " + saving + "</div>") if saving else ""}'
...
def _render_fallback_recommendations(r: dict) -> None:
    """Fallback jika Claude API tidak tersedia."""
    st.info("Showing rule-based recommendations as fallback.")
    sorted_ap = sorted(
